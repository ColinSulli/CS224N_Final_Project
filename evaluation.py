    ...
    with torch.no_grad():
        ...
        if arg == 'para' or arg == 'all':
            for step, batch in enumerate(tqdm(paraphrase_dataloader, desc=f'para eval', disable=TQDM_DISABLE)):
                (b_ids1, b_token_type_ids_1, b_mask1,
                 b_ids2, b_token_type_ids_2, b_mask2,
                 b_labels, b_sent_ids) = (batch['token_ids_1'], batch['token_type_ids_1'], batch['attention_mask_1'],
                              batch['token_ids_2'], batch['token_type_ids_2'], batch['attention_mask_2'],
                              batch['labels'], batch['sent_ids'])

                b_ids1 = b_ids1.to(device)
                b_mask1 = b_mask1.to(device)
                b_token_type_ids_1 = b_token_type_ids_1.to(device)
                b_ids2 = b_ids2.to(device)
                b_token_type_ids_2 = b_token_type_ids_2.to(device)
                b_mask2 = b_mask2.to(device)

                logits = model.predict_paraphrase(b_ids1, b_token_type_ids_1, b_mask1, b_ids2, b_token_type_ids_2, b_mask2)
        ...
        if arg == 'sts' or arg == 'all':
            for step, batch in enumerate(tqdm(sts_dataloader, desc=f'sts eval', disable=TQDM_DISABLE)):
                ...
                logits = model.predict_similarity(b_ids1, b_token_type_ids_1, b_mask1, b_ids2, b_token_type_ids_2, b_mask2)
...
def model_eval_test_multitask(sentiment_dataloader,
    ...
    with torch.no_grad():
        ...
        for step, batch in enumerate(tqdm(paraphrase_dataloader, desc=f'para test eval', disable=TQDM_DISABLE)):
            (b_ids1, b_token_type_ids_1, b_mask1,
                 b_ids2, b_token_type_ids_2, b_mask2,
                 b_sent_ids) = (batch['token_ids_1'], batch['token_type_ids_1'], batch['attention_mask_1'],
                              batch['token_ids_2'], batch['token_type_ids_2'], batch['attention_mask_2'],
                              batch['sent_ids'])
